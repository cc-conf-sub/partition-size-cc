...
public class DNode {
    ...
    // ------------------------------------------------------------
    // FAST VOTE ALGORITHM
    // Uses new data structure to speed up finding min size cluster

    public static double[] fast_min_prob_edit_dist_choice_rn(int cur_node, ArrayList<ArrayList<Integer>> cur_clustering, int k, int min_cluster_size, int min_cluster_index, HashMap<Integer, Integer> settled_nodes, ArrayList<ArrayList<Integer>> prob_matrix) {
        // compute clustering choice that minimizes increase of prob edit dist
        int num_clusters = cur_clustering.size();         
        ArrayList<Double[]> avg_weights = new ArrayList<Double[]>();
        ArrayList<Integer> cur_edges = prob_matrix.get(cur_node);
    ...
    public static ArrayList<ArrayList<Integer>> fast_k_vote(ArrayList<ArrayList<Integer>> prob_matrix, int k) {
        /*
            RandomNode ordering
            Assume 0/1 graph; Make use of Neighborhood oracle provided  
        */
        ...
        ArrayList<HashSet<Integer>> cluster_sizes = new ArrayList<HashSet<Integer>>(num_nodes);
        ...
        while (settled_nodes.size() < num_nodes) {
            ...
            double[] max_cluster = fast_min_prob_edit_dist_choice_rn(cur_node, cur_clustering, k, min_cluster_size, min_cluster_index, settled_nodes, prob_matrix);
    
            if (max_cluster[0] >= 0.5 && cur_clustering.size() == k) {
                // add cur_node to the clustering
                int new_index = (int) max_cluster[1];
                int old_size = cur_clustering.get(new_index).size();

                cur_clustering.get(new_index).add(cur_node);
                settled_nodes.put(cur_node, new_index);

                // INCREMENT CLUSTER SIZE
                cluster_sizes.get(old_size).remove(new_index);
                cluster_sizes.get(old_size + 1).add(new_index);

                // UPDATE POINTERS
                while (cluster_sizes.get(min_cluster_size).isEmpty())
                    min_cluster_size++; 
                if (!cluster_sizes.get(min_cluster_size).contains(min_cluster_index))
                    // not liking the complexity of this operation, but hopefully set sizes are small
                    min_cluster_index = cluster_sizes.get(min_cluster_size).iterator().next();
                ...
                if (cur_clustering.size() == k) {
                    // INITIALIZE DATA STRUCTURE FOR MIN CLUSTER SIZE
                    for (int i = 0; i < num_nodes; i++)
                        cluster_sizes.add(i, new HashSet<Integer>());
                    for (int i = 0; i < k; i++) {
                        int cur_size = cur_clustering.get(i).size(); 
                        cluster_sizes.get(cur_size).add(i);
    ...
    // Vote then randomly assign -- to avoid heap 
    // Wish me luck!


    public static ArrayList<ArrayList<Integer>> k_vote_then_random(ArrayList<ArrayList<Integer>> prob_matrix, int k) {
        /*
            RandomNode ordering // TODO: use same order as corresponding Pivot clustering?
            Assume 0/1 graph; Make use of Neighborhood oracle provided
        */
        ...
        while (settled_nodes.size() < num_nodes) {
            ...
            if (cur_clustering.size() == k) {
                ...
                int randIndex = random.nextInt(k);
                cur_clustering.get(randIndex).add(cur_node);
                settled_nodes.put(cur_node, randIndex);
