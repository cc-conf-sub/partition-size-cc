...
public class KLocalSearch {
	...
	*/
    ...
    // FAST VERSION:  uses new data structure to track min cluster size

    private static double[] fast_min_cost_choice_ls(int cur_node, ArrayList<Integer> cluster_sizes, int min_cluster_size, int min_cluster_index, HashMap<Integer, Integer> cluster_map, ArrayList<ArrayList<Integer>> prob_matrix) {
        // compute clustering choice that minimizes increase of prob edit dist
        int num_clusters = cluster_sizes.size();         
        ArrayList<Double[]> avg_weights = new ArrayList<Double[]>();
        ArrayList<Integer> cur_edges = prob_matrix.get(cur_node);
        ...
        if (!pos_connections.keySet().contains(min_cluster_index)) {
            // add one more choice to consider

            if (min_cluster_index != cluster_map.get(cur_node)) { 
                Double[] cur_val = {(double) (min_cluster_size + total_connections), (double) min_cluster_index};
                cost_increase.add(cur_val); 
            } else {
                Double[] cur_val = {(double) (min_cluster_size - 1 + total_connections), (double) min_cluster_index};
                cost_increase.add(cur_val);
            }
        }
    ...
    public static ArrayList<ArrayList<Integer>> fast_k_local_search(ArrayList<ArrayList<Integer>> prob_matrix, int k, double threshold) {
        /*
            Make one pass through given clustering, using "best of one-element moves" strategy
            Assume 0/1 graph; Make use of Neighborhood oracle provided  
        */
        ...
        ArrayList<ArrayList<Integer>> cur_clustering = initializeClustering(num_nodes, k);
        if (threshold == 0)
            return cur_clustering; // all done!
        ...
        ArrayList<Integer> cluster_sizes = new ArrayList<Integer>();
        ArrayList<HashSet<Integer>> sorted_cluster_sizes = new ArrayList<HashSet<Integer>>(num_nodes);
	for (int i = 0; i < num_nodes; i++)
	    sorted_cluster_sizes.add(i, new HashSet<Integer>());
        int min_cluster_size = num_nodes;
        int min_cluster_index = -1;

        for (int i = 0; i < cur_clustering.size(); i++) {
            int cur_size = cur_clustering.get(i).size();
            cluster_sizes.add(cur_size);
            sorted_cluster_sizes.get(cur_size).add(i);
            if (cur_size < min_cluster_size) {
                min_cluster_size = cur_size;
                min_cluster_index = i;
            }
        }
        ...
        while(true) { // while not converged...
            for (int i = 0; i < num_nodes; i++) {
                ...
                Integer cur_node = ecs_list.get(i);        
                double[] max_cluster = fast_min_cost_choice_ls(cur_node, cluster_sizes, min_cluster_size, min_cluster_index, cluster_map, prob_matrix);
                ...
                if (max_cluster[0] >= 0.5) {    
                    ...
                    if (old_index != new_index) {
                        ...
                        int old_cluster_size = cluster_sizes.get(old_index);
                        int new_cluster_size = cluster_sizes.get(new_index);

                        cluster_map.put(cur_node, new_index);
                        cluster_sizes.set(old_index, old_cluster_size - 1);
                        cluster_sizes.set(new_index, new_cluster_size + 1);

                        cur_clustering.get(old_index).remove(cur_node);
                        cur_clustering.get(new_index).add(cur_node);

                        // UPDATE min cluster size
                        sorted_cluster_sizes.get(old_cluster_size).remove(old_index);
                        sorted_cluster_sizes.get(old_cluster_size - 1).add(old_index);
                        if (old_cluster_size - 1 < min_cluster_size) {
                            min_cluster_size = old_cluster_size - 1;
                            min_cluster_index = old_index;
                        } 

                        sorted_cluster_sizes.get(new_cluster_size).remove(new_index);
                        sorted_cluster_sizes.get(new_cluster_size + 1).add(new_index);

                        // UPDATE POINTERS
                        while (sorted_cluster_sizes.get(min_cluster_size).isEmpty())
                            min_cluster_size++; 
                        if (!sorted_cluster_sizes.get(min_cluster_size).contains(min_cluster_index))
                            // not liking the complexity of this operation, but hopefully set sizes are small
                            min_cluster_index = sorted_cluster_sizes.get(min_cluster_size).iterator().next();
	...
	/*
        ...
        for (int i = 0; i < cur_clustering.size(); i++) {
            if (cur_clustering.get(i).size() != 0)
                new_clustering.add(cur_clustering.get(i));
        }
